...
class Traffic extends Common_functions {
    ...
    public function get_interface_history($device_id, $if_index, $timespan = '1d') {
        ...
        // 修正：使用 if_index 字段作为查询条件，之前可能会有不同的字段名
        $query = "SELECT 
        ...
        try {
            ...
            // 添加新的调试日志，记录原始参数值
            error_log("调试: 原始参数类型: device_id=".gettype($device_id).", if_index=".gettype($if_index));
            error_log("调试: 原始参数值: device_id=$device_id, if_index=$if_index");
            
            // 确保参数类型正确
            $device_id = (int)$device_id;
            $if_index = (string)$if_index;
            
            // 添加缓存破坏参数，确保不使用过期数据
            $cacheBreaker = time();
            error_log("调试: 使用缓存破坏参数: $cacheBreaker");
